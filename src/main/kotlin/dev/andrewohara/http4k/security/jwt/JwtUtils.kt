...
fun http4kResourceRetriever(http: HttpHandler) = ResourceRetriever { url ->
    val response = Request(Method.GET, url.toString()).let(http)

    // Nimbus will interpret an IOException as a JWKSetRetrievalException
    if (!response.status.successful) {
        throw IOException("Error retrieving JWK from $url: $response")
    }
